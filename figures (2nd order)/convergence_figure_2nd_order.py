"""
Robin-BC convergence figures, WITH a second-order reference line.
...
Writes, relative to this file:
    sphere/sphere_convergence_2nd_order.pdf | .png
    star/star_convergence_2nd_order.pdf     | .png
    star3/star3_convergence_2nd_order.pdf   | .png
...
for folder, title, L, rad, d in SHAPES:
    ...
    # Both guides are anchored ON the L_inf value at Nx=16 -- the start of the
    # resolved range -- and span 16..64. A reference line's vertical offset is
    # arbitrary, so anchoring it to a data point is the only way the comparison
    # means anything: the question becomes "from this point, does the data follow
    # the first- or the second-order line?" Anchored this way, all three
    # geometries end up ABOVE the second-order line at Nx=64 (2.2x-3.9x) and
    # track the first-order line instead.
    xa = float(nxs[1])
    xr = np.array([xa, float(nxs[-1])])
    y0 = d[nxs[1]][0]
    ax.plot(xr, y0 * (xa / xr), ls=(0, (6, 4)), lw=1.2,
            color="black", label=r"$\mathcal{O}(N_x^{-1})$", zorder=1)
    ax.plot(xr, y0 * (xa / xr) ** 2, ls=(0, (1.5, 2.5)), lw=1.2,
            color="0.45", label=r"$\mathcal{O}(N_x^{-2})$", zorder=1)
    ...
    ax.set_ylim(min(10 ** (math.floor(math.log10(lo)) - 0.15),
                    y0 * (xa / nxs[-1]) ** 2 / 1.6),
    ...
    stem = os.path.join(out, f"{folder}_convergence_2nd_order")
    for ext in ("png",):
        fig.savefig(f"{stem}.{ext}", dpi=220, bbox_inches="tight")
    plt.close(fig)
    print(f"{title:7s} order  L_inf {o_i:.2f}  RMSE {o_r:.2f}   ->  {folder}/{folder}_convergence_2nd_order.pdf|png")
